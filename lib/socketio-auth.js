...
module.exports = function socketIOAuth(io, config) {
  ...
  io.on('connection', function(socket) {
    ...
    socket.on('authentication', function(data, ack) {
      config.authenticate(socket, data, function (err, success, extraData) {
        if(err instanceof Error)
          err = {message: err.message, code:err.code}

        if(ack) ack(err, success);
        ...
        if (success) {
          ...
          return postAuthenticate(socket, data, extraData);
        } else if (err) {
          ...
          let err = {message: 'Authentication failure', code:'UNAUTHORIZED'};
          socket.emit('unauthorized', err, function() {
            socket.disconnect();
          });
        }
